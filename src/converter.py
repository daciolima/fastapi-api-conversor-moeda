...
# import lib async para chamadas request. 
import aiohttp
...
def sync_converter(from_currency: str, to_currency: str, price: float):
    ...
    if "Realtime Currency Exchange Rate" not in data:
        raise HTTPException(status_code=400, detail='Realtime Currency Exchange Rate not in response {data}')
